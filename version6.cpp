...
bitmap_file height_bmp_file, colour_bmp_file;           
...
int heights[512][512];
int colours[512][512];
int x_pos = 256, y_pos = 256, z_pos = 6000;
double z_delta = -360.0 / 10.0, fov = 0.0;
	
	
int flag = 0, poo = 5, min,counter = 0;
...
void GameMain() {
	...
	for (int col = 0; col < 640; col++) {
		...
		dx = cos(y_angle*PIE*2.0/3840.0) ;  
		dy = sin(y_angle*PIE*2.0/3840.0) ;
		...
		for (int curr_step = 0; curr_step < 1000; curr_step++)
			{
...
int DD_Init(HWND hwnd)
{
	...
	for(index = 0; index < 256; index++ ) 
	{
			...
			colour_palette[index].peRed = 0;//colour_bmp_file.palette[index].peBlue ;
			colour_palette[index].peBlue = 0;//colour_bmp_file.palette[index].peRed;
			colour_palette[index].peGreen = index;//colour_bmp_file.palette[index].peGreen;
	...
	colour_palette[index].peBlue = 50 + index;//colour_bmp_file.palette[index].peRed;
	colour_palette[index].peGreen = 0;
	}
...
int WINAPI WinMain(HINSTANCE hThisInst, HINSTANCE hPrevInst, LPSTR lpszArgs, int nWinMode)
{
	...
	LoadBitmap(&colour_bmp_file,"try.bmp");
...
//			if ((colours[i][j] = colour_bmp_file.buffer[index++])>250) colours[i][j] = 250;
			colours[i][j] = (((double)(heights[i][j]) - min) / (double) poo ) * 200.0;
//			if (colours[i][j] < 20) colours[i][j] = 20;
...
void LoadDEM() {
	...
	int profile, points, templow = 7000, temphigh = 0;
	char ch = 0;
	
	ifstream fin("rockville.txt");
