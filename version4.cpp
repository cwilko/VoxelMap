...
int heights[512][512];
int colours[512][512];
int x_pos = 2, y_pos = 256, z_pos = 6000;
double pitch = -(SCREEN_HEIGHT/2);
int fov = 320;
...
double inc = 1.0/10.0;
double convert = 2.0 * PIE / 3840.0;

void Render() {

	double x,y,z,dx,dy,m,scale,y_angle;
	...
	if (KEY_DOWN(VK_UP)) {
	x_pos+=4 * cos(fov * convert);
	y_pos+=4 * sin(fov * convert);
	...
	y_angle = (double) fov + 320.0;
	
	for (int col = 0; col < SCREEN_WIDTH; col++) {
	
		x = x_pos; y = y_pos; z = z_pos;
	
		dx = cos(y_angle * convert);
		dy = sin(y_angle * convert);
		...
		for (int i = 0; i < MAX_STEPS; i++)
			...
			if (x > MAP_WIDTH || x<0 || y > MAP_HEIGHT || y<0) height = 0, colour = 0;
			else {
				height = (heights[(int)x][(int)y] - poo) * 4  ;
...
int WINAPI WinMain(HINSTANCE hThisInst, HINSTANCE hPrevInst, LPSTR lpszArgs, int nWinMode)
{
	...
	LoadBitmap(&colour_bmp_file,"lassen.bmp");
...
//			colours[i][j] = ((double)heights[i][j] / (double) poo) * 200.0;
...
//			heights[i][j] = height_bmp_file.buffer[index++] * 8;
	...
	while (1) 
	{
		...
		sprintf(buffer,"%2.0f",1/duration);
...
void LoadDEM() {
	...
	int profile, points, templow = 6000, temphigh = 0;
	char ch = 0;
